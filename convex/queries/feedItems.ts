...
export const getFeedItemsForOfficeService = internalQuery({
  ...
  handler: async (ctx, { officeId, serviceId }) => {
    // Get all sources applicable to this office+service (all 4 scopes)
    const [globals, serviceScoped, officeScoped, officeServiceScoped] = await Promise.all([
      ctx.db.query("sources").withIndex("by_scope", q => q.eq("scope", "global"))
        .filter(q => q.eq(q.field("active"), true)).collect(),
      ctx.db.query("sources").withIndex("by_service", q => q.eq("serviceId", serviceId))
        .filter(q => q.and(q.eq(q.field("scope"), "service"), q.eq(q.field("active"), true))).collect(),
      ctx.db.query("sources").withIndex("by_office", q => q.eq("officeId", officeId))
        .filter(q => q.and(q.eq(q.field("scope"), "office"), q.eq(q.field("active"), true))).collect(),
      ctx.db.query("sources").withIndex("by_office", q => q.eq("officeId", officeId))
        .filter(q => q.and(
    ...
    const allSources = [...globals, ...serviceScoped, ...officeScoped, ...officeServiceScoped];
    ...
    // Also collect authority items directly scoped to this office+service
    const authorityItems = await ctx.db.query("feedItems")
      .withIndex("by_office_service", q =>
        q.eq("officeId", officeId).eq("serviceId", serviceId)
      ).collect();
